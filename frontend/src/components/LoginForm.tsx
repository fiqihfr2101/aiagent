...
export default function LoginForm({ onLoginSuccess }: LoginFormProps) {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    if (requires2FA) {
      if (!totpCode.trim() || totpCode.length !== 6) {
        setLocalError('Please enter a valid 6-digit 2FA code');
        return;
      }
    }
    ...
    try {
      const result = await login(username.trim(), password, requires2FA ? totpCode.trim() : undefined);
  ...
  }, [username, password, totpCode, requires2FA, login, clearError, onLoginSuccess]);
      ...
      <div className="relative z-10 w-full max-w-md px-6">
        ...
        <div className="bg-[#111827]/80 backdrop-blur-sm border border-gray-700/50 rounded-xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {/* Username - hidden when 2FA is required */}
            {!requires2FA && (
              <>
                <div>
          ...
          {/* Back to login when in 2FA mode */}
          {requires2FA && (
            <div className="mt-4">
              <button
